...
**Page volume (advanced mode, large repos only).** Above ~2,000 documentable files, advanced mode asks whether to bound the file-page bucket: the top 2,000 files by importance, or one page per eligible file. Below that the question is never asked and every eligible file gets a page. File pages are rendered from structure and cost no model tokens, so what the bound saves is wiki size, embedding calls and retrieval noise, and both answers are quoted in pages and megabytes before you pick. The answer is saved as `max_file_pages` in `config.yaml`, which `update --full` and `generate` honour; set it there directly (no terminal needed) for a scripted or hosted run.
